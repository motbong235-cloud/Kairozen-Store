...
--------------------------------------
Serves index.html (the single-file store, unchanged) and adds two tiny
backend routes that the store's JS already calls: /generate and
/check_payment. They proxy to ABA PayWay via khmer-system.com — same
integration as baby_server.py's aba_generate_qr()/aba_check_payment(),
trimmed down to just what this storefront needs.
